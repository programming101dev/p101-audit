...
static bool is_allowed(const struct p101_env *env, struct p101_wrapper_arguments *arguments, const struct p101_wrapper_fact *fact, const struct p101_wrapper_inventory *wrapper)
{
     ...
     */
    /*
     * The wrap itself: p101_x calling native x, judged from the inventory so
     * no library has to declare it in an allow file. A wrapper must call the
     * native function it wraps; that call is the wrap, not a missed one.
     */
    allowed = false;
    if(wrapper != NULL)
    {
