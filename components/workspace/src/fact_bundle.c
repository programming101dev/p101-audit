...
bool p101_workspace_fact_bundle_load(const struct p101_env *env, struct p101_error *err, const char *path, struct p101_wrapper_model *model)
{
    ...
    *next_line = '\0';
    ...
    while(*line != '\0')
    {
        ...
        copied     = copy_bundle_field(env, err, fact->name, sizeof(fact->name), fields[BUNDLE_NAME]) && copied;
        copied     = copy_bundle_field(env, err, fact->usr, sizeof(fact->usr), fields[BUNDLE_USR]) && copied;
        copied     = copy_bundle_field(env, err, fact->caller_usr, sizeof(fact->caller_usr), fields[BUNDLE_CALLER_USR]) && copied;
        copied     = copy_bundle_field(env, err, fact->resolved, sizeof(fact->resolved), fields[BUNDLE_RESOLVED]) && copied;
        copied     = copy_bundle_field(env, err, fact->replacement, sizeof(fact->replacement), fields[BUNDLE_REPLACEMENT]) && copied;
        copied     = copy_bundle_field(env, err, fact->type, sizeof(fact->type), fields[BUNDLE_TYPE]) && copied;
        copied     = copy_bundle_field(env, err, fact->canonical_type, sizeof(fact->canonical_type), fields[BUNDLE_CANONICAL_TYPE]) && copied;
        copied     = copy_bundle_field(env, err, fact->return_type, sizeof(fact->return_type), fields[BUNDLE_RETURN_TYPE]) && copied;
        copied     = copy_bundle_field(env, err, fact->caller, sizeof(fact->caller), fields[BUNDLE_CALLER]) && copied;
        copied     = copy_bundle_field(env, err, fact->parent_usr, sizeof(fact->parent_usr), fields[BUNDLE_PARENT_USR]) && copied;
        parsed     = parse_bundle_size(fields[BUNDLE_LINE], &fact->line);
        parsed     = parse_bundle_size(fields[BUNDLE_COLUMN], &fact->column) && parsed;
        parsed     = parse_bundle_size(fields[BUNDLE_START], &fact->start) && parsed;
        parsed     = parse_bundle_size(fields[BUNDLE_END], &fact->end) && parsed;
        parsed     = parse_bundle_size(fields[BUNDLE_PARAMETER_INDEX], &fact->parameter_index) && parsed;
        parsed     = parse_bundle_flag(fields[BUNDLE_IS_DEFINITION], &fact->is_definition) && parsed;
        parsed     = parse_bundle_flag(fields[BUNDLE_IS_HEADER], &fact->is_header) && parsed;
        parsed     = parse_bundle_flag(fields[BUNDLE_IS_STATIC], &fact->is_static) && parsed;
        parsed     = parse_bundle_flag(fields[BUNDLE_IS_PUBLIC], &fact->is_public) && parsed;
        parsed     = parse_bundle_flag(fields[BUNDLE_IS_VARIADIC], &fact->is_variadic) && parsed;
        parsed     = parse_bundle_flag(fields[BUNDLE_IS_LOCAL], &fact->is_local_include) && parsed;
        parsed     = parse_bundle_flag(fields[BUNDLE_IS_INDIRECT], &fact->is_indirect) && parsed;
        parsed     = parse_bundle_flag(fields[BUNDLE_NEEDS_ENV], &fact->needs_env) && parsed;
        parsed     = parse_bundle_flag(fields[BUNDLE_NEEDS_ERROR], &fact->needs_error) && parsed;
        if(!copied || !parsed)
        {
